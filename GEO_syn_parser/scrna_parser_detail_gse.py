...
model_disease = pd.read_excel("dc_disease.xls")
model_cellpops = pd.read_excel("dc_cellpops.xls")
model_celltypes = pd.read_excel("dc_celltypes.xls")
model_celllines = pd.read_excel("dc_lines.xls")
model_papers    = pd.read_excel("dc_papers.xls")
model_tissues   = pd.read_excel("dc_tissues.xls")
model_journal   = pd.read_excel("dc_journals.xls")
...
def update_one_sample(gseid, ddir='geo', parse_fields=['other_ids', 'paper', 'name', 'species', 'description', 'factor',
    ...
    res = [gseid, str(species), str(pmid), str(paper), str(name), str(tmp_celltype),
        ...
        str(geo_last_update_date), str(srx), str(cell_line), str(accession), str(xmlContent)]
    time.sleep(3)
    return res
