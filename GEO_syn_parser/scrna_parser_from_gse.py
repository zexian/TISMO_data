...
def getGDSSamples(date_region=False):
    ...
    #REAL URL
    URL = """http://eutils.ncbi.nlm.nih.gov/entrez/eutils/esearch.fcgi?db=gds&term=SRA[Sample%20Type]%20AND%20gse[Entry%20Type]%20AND%20(mus%20musculus[Organism])&retmax=100000&usehistory=y"""
    if date_region:
        maxTime = date_region.split('-')[1]
        minTime = date_region.split('-')[0]
        URL = """http://eutils.ncbi.nlm.nih.gov/entrez/eutils/esearch.fcgi?db=gds&term=SRA[Sample%%20Type]%%20AND%20gse[Entry%20Type]%20AND%20(mus%20musculus[Organism])&mindate={0}&maxdate={1}&datetype=pdat&retmax=100000&usehistory=y""".format(minTime, maxTime)
    try:
...
def _sync_gse(fsave, fill_or_not=False, DataType=False, dateRegion = False, refresh=False, exludeFile = False, xmlPath = './geo_gse'):
    ...
    #f = open(fsave, 'a')
    #f.write('%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s'%('GSE', 'Species',
    #    'PMID', 'Paper', 'Title', 'CellType', 'Tissue', 'Disease',
    #    'Cell_Pop', 'Release_Date', 'Last_Update_Date', 'GSMs', 'Cell_Line', 'fields_dataType', 'Key_Match')+'\n')
    #f.close()
    ...
    for gds in gdsSamples:
        ...
        getType = getGEOSamples_byType_gse.getGeoSamples_byTypes(path = "repository_samples.pickle", datatype = ['rna-seq'],
...
def sync_samples_from_gse_factor(infile, gse_col, fsave, xmlPath='geo_gse', exludeFile = False, fill_or_not = False, refresh = False):
    ...
    f.write('%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s'%('GSE', 'Species',
        'PMID', 'Paper', 'Title', 'CellType', 'Tissue', 'Disease',
        'Cell_Pop', 'Release_Date', 'Last_Update_Date', 'GSMs', 'SRA', 'fields_dataType', 'Key_Match')+'\n')
    f.close()
    exclude = []
    if exludeFile:
        exclude = [x.rstrip() for x in open(exludeFile)]
...
def getLocalGeo(fsave, fill_or_not=False, xmlPath="geo_gse", DataType=False, refresh = False):
    ...
    for gseid in local_repo_samples:
        ...
        getType = getGEOSamples_byType_gse.getGeoSamples_byTypes(path = "repository_samples.pickle", datatype = ['rna-seq'], ddir=xmlPath, gseids=[gseid], refresh=refresh)
        if getType:
            for i in getType.keys():
                # parse sample annotation
                list_sample = scrna_parser_detail_gse.update_one_sample(gseid=gseid, ddir=xmlPath)
