...
def main():
    try:
        parser = argparse.ArgumentParser(description="""rnaseq dataset parser from GEO""")
        ...
        new_parser.add_argument('-p', dest="path_of_xml", type = str, required = False, help = 'The folder path contain all the xml files, eg: "./geo", the xml storage format should be: "./geo/GSE1000/GSE1000102.xml".')
        ...
        if args.sub_command == 'parser':
            dregion, file_save, fill_or_not, excludes, path_of_xml = args.date_region, args.fsave, args.fill, args.exclude, args.path_of_xml
            # check date region
            if not dregion:
            ...
            #check save file    
            if not file_save:
                file_save = './rnaseq_new_collection.xls'
            ...
            for oneTime in checkTime:
                ...
                if fill_or_not:
                    ...
                    _sync_gse(file_save, fill_or_not, dateRegion = dregion, exludeFile=excludes, refresh=True, xmlPath = path_of_xml)
